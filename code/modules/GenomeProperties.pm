...
sub set_options {
  ...
  my $input = 0;
  $input++ if (defined($options->{matches}));
  $input++ if ($options->{matches_dir});
  ...
  croak("Need the extension of the files") if (!$options->{ext} and $options->{matches_dir});
  warn "Extension not needed. Ignoring it\n" if ($options->{ext} and !$options->{matches_dir});

  my $calculation = 0;
  $calculation++ if (defined($options->{all}));
  $calculation++ if ($options->{list});
  $calculation++ if ($options->{property});

  if ($calculation > 1){
	die "Only one form of calculation please\n";
    }
  elsif($calculation == 0){
	die "Please provide a form of calculation\n";
    }
...
sub define_sequence_set {
  ...
  if($seq_blob){
    ...
    if ($self->{seqs}) {
      ...
      open(my $FH, '<', $self->{seqs}) or croak( "Failed to open ".$self->{seqs}."\n");
      @sb = <$FH>;
      close $FH;
      foreach my $l (@sb){
        $self->{seqs_and_annotations}->{$1} = [] if ($l =~ /^>(\S+)/);
        }
      }
    elsif ($self->{seqs_dir}) {
...
sub signature_matches {
  ...
  elsif(!$self->{read_sig} && $self->{matches}){
    my @mb;
    my $species = $self->{matches};
    open(my $FH, '<', $species) or die "Could not open signature matches file\n";
    @mb = <$FH>;
    close($FH);
...
sub check_results{
  ...
  foreach my $acc (@prop_list){
    ...
    next if ((defined $self->{_to_evaluate}) && (scalar @{$self->{_to_evaluate}} > 0) && (!grep {/$acc/} @{$self->{_to_evaluate}}));
...
sub evaluate_property {
  ...
  foreach my $step (@{ $def->get_steps }){
   ...
   if($step->{found}){
      $found++;
      if ($self->{minimumFH}) {
        push (@{$minimum{$step->{order}}{members}}, @{$step->found});  # For each step, we store the array of species where the step was found
        # To calculate the minimum path, need to distinguish between sufficient and non sufficient steps, because they will have different weights 
    ...
    else { # Missing non requerid steps
      push (@{$minimum{$step->{order}}{members}}, "MISSING*") if ($self->{minimumFH});
      }
  ...
  if(($found == 0) or ($found <= $def->threshold)){
    $def->result('NO'); #Not enough required steps found
    return;
    }
  elsif($missing > 0){
...
sub evaluate_step {
  my($self, $step) = @_;
  my @succeed; # array of species where the step was found
  my @insufficients; # will store the species where non 'sufficient' evidences are found
  my $insufficientEvidence = 0; # will store the number of non 'sufficient' evidences found
  my $number_evidences;  # will store the number of not 'sufficient' evidences
  ...
  if (!scalar(@{$evRef})){
    ...
    $number_evidences = scalar @{$evRef}; # First we get the total number of evidences in the GP, and then we subtract the 'sufficient' ones
    foreach my $evObj (@{$evRef}){
      
      if($evObj->gp){
        print "\tevidence is ".$evObj->gp."\n" if ($self->{debug});
        push (@{$self->{_to_evaluate}}, $evObj->gp) if ((defined $self->{_to_evaluate}) && (!grep {/$evObj->gp/} @{$self->{_to_evaluate}}));
        ...
        my @current_evidence; # Stores the species for each evidence in a step. Then just the species that have all the non 'sufficient' EVs will be considerer as success
        ...
        if($self->get_family($evObj->accession) and $self->{families}{$evObj->accession} != 0) {
          ...
          if(defined($evObj->type) and $evObj->type eq 'sufficient'){ # if the evidence is sufficient, store in @succeed
            ...
            $insufficientEvidence++; # Number of non 'sufficient' evidences found
            ...
            @insufficients = intersect (@insufficients, @current_evidence); # In each iteration we discard the species that lack some evidence
            }    
          }
        }
      else{
...
sub print_long {
  my($self, $prop) = @_;
  my %species_prop; # Stores all the species and the number of required steps found
  ...
  foreach my $step (sort { $a->order <=> $b->order} @{ $prop->get_steps }){
    ...
    if ($step->found) {
      if($self->{matches}){
        $report .= ".\tSTEP RESULT: YES\n";
        }
      elsif($self->{matches_dir}){
        $self->find_step_results ($step, \@species_step);
  ...
  foreach my $found (@{$step->found}) {
    if ($found =~ /^GenProp/) {
      ...
      foreach my $new_step (sort { $a->order <=> $b->order} @{ $new_prop->get_steps }){
        $self->find_step_results ($new_step, $species_found);
        }
      }
    else {
...
sub print_table {
  ...
  foreach my $step (sort { $a->order cmp $b->order} @{ $prop->get_steps }){
    ...
    if (($step->found != 0) && ($self->{matches})) {
      $report .= $row."\t".$step->order."\t".$step->step_name."\tYES\t".(defined($step->required) ? "YES" : "NO")."\t$evidence_type\n";
      }  
    elsif (($step->found != 0) && ($self->{matches_dir})) {
